...
onMounted(async () => {
  if (props.courseId && props.courseId !== 'new') {
...
async function saveCourse() {
  ...
  try {
    if (id) {
      ...
      if (error) throw error
      if (data) {
        // Update form with new course ID
...
function manageLessons() {
  if (course.value) {
    router.go(`/admin/lessons?courseId=${course.value.id}`)
  }
}
</script>
    ...
    <form v-else @submit.prevent="saveCourse" class="editor-form">
      <div class="form-layout">
        ...
        <aside class="sidebar-column">
          ...
          <div v-if="course" class="sidebar-section metadata">
